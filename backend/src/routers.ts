...
import { EndpointNotFound, RestError } from "./core/errors";
import { corsApi } from "./core/restApi";
...
function createApiVer1Router(db: DB) {
  ...
  registerRoute(router, db, {
    ...
    async requestHandler(ctx): Promise<Endpoints['/api/v1/user/getHomeTimeline']['result']> {
      ...
      const result = await LeafsQueryService.fetchHomeTimeline(params2, { userId: ctx.getUser().userId }, ctx.db);
      return result;
    },
  });
